            ...
            if mask_groups:
                ...
                for i, group in enumerate(mask_groups):
                    ...
                    for cx1, cy1, cx2, cy2 in work.sfx_carve_bboxes:
                        # Carve out exactly the SFX item's bbox - no outward
                        # padding. Padding this rectangle used to leave a thin
                        # ring of untouched original pixels around the SFX
                        # region, which shows up as a visible seam/margin
                        # line against the inpainted art next to it (and
                        # makes the region look slightly cut/shrunk). Keeping
                        # the carve exact avoids that artifact while still
                        # guaranteeing the SFX pixels themselves are never
                        # touched.
                        cx1c = max(0, cx1)
                        cy1c = max(0, cy1)
                        cx2c = min(img_w, cx2)
                        ...
                        if ow > 0 and oh > 0:
                            ...
                            if sx2 > sx1 and sy2 > sy1:
                                ...
                                if border_pixels is not None and border_pixels.size > 0:
                                    ...
                                    # Real scans/screenshots rarely hit pure
                                    # 255/0 due to JPEG compression, paper
                                    # texture, or subtle anti-aliasing near
                                    # panel edges. A too-strict threshold here
                                    # makes genuinely solid white/black panels
                                    # fail the "is solid" check, so they fall
                                    # through to texture inpainting (LaMa/CV2)
                                    # instead of a clean flat fill - producing
                                    # a visible grey/tinted box on what should
                                    # be plain white or black.
                                    white_thresh = 235
                                    black_thresh = 20

...
# "Copy Original Text Style" (match_original_style) should reproduce the
# original text's actual size - including large stylized title/SFX text -
# rather than being silently capped by the user's normal --max-font-size
# (which is tuned for regular dialogue bubbles, commonly as low as 16px).
# This is a separate, generous ceiling used only when scaling a measured
# target_font_size, so oversized/corrupt measurements still can't runaway.
STYLE_MATCH_TARGET_FONT_SIZE_CEILING = 256
            ...
            if config.rendering.match_original_style:
                ...
                if target_font_size is not None:
                    target_font_size = scale_font_size(
                        ...
                        processing_scale,
                        minimum=osb_min_font,
                        maximum=STYLE_MATCH_TARGET_FONT_SIZE_CEILING,
                    )
